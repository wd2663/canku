...
exports.shop = function (req, res, next) {
  db.shop.findOne({'_id': db.ObjectID.createFromHexString(req.params.id)}, function (err, shop) {
    if (!err) {
      ...
      db.food.find({'shop_id': req.params.id, week: {$in: ['-1', week]}}).sort({category: 1, count_order: -1}).toArray(function (err, foods) {
        if (!err) {
    ...
    db.shop.findAndModify({_id: db.ObjectID.createFromHexString(req.params.id)},[],{$inc:{count_visit:1}},{new:true,upset:true},function(error,result){
...
exports.submit_order = function (req, res) {
  ...
  for (var i in order_list) {
    total = total + ( parseFloat(order_list[i].price) * parseInt(order_list[i].num));
      ...
      db.food.findAndModify({_id:  db.ObjectID.createFromHexString(order_list[i].id)},[],{$inc:{count_order:1}},{new:true,upset:true},function(error,result){
          console.log('=======>', error, result);
      });
  ...
  db.order.insert({
    ...
    db.shop.findAndModify({_id: db.ObjectID.createFromHexString(req.body.shop_id)},[],{$inc:{count_order:1}},{new:true,upset:true},function(error,result){
        console.log('=======>', error, result);
    });
